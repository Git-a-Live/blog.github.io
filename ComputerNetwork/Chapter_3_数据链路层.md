...
MAC帧（也称以太网帧）的格式如下图所示：

![](pics/mac_frame.png)

从图中可以看出，MAC帧的结构较为简单，首部由三个字段、尾部由一个字段组成。
...
和总线一样，集线器也是工作在**物理层**，其接口仅仅是简单地转发比特，而不进行任何碰撞检测，而且通常具有少量的容错能力和网络管理功能。常见的集线器通常是由4~8个集线器堆叠起来使用的**堆叠式**集线器，一些模块化的机箱式集线器还支持功能模块的热插拔。
...
IEEE 802.1Q标准对于虚拟局域网（Virtual LAN，VLAN）的定义是：由一些具**有某些共同需**求的局域网网段构成的，与地理位置无关的**逻辑组**。

VLAN并不是什么新型的局域网，而是原有的以太网利用以太网交换机，对用户和网络资源重新进行逻辑组合。在同一个VLAN当中，任何一台主机发送的广播信息只会在所属的VLAN工作组内传送，不会传播到工作组以外的范围，从而防止整体网络因传播过多的广播信息（广播风暴）而引起性能恶化。

VLAN当中发送的数据帧是在普通MAC帧的基础上进行扩展，在首部加入了4字节的VLAN标记字段（这样MAC帧的最大字节数就从1518增加到1522），用于标记该帧是一个VLAN帧，以及来源于哪个VLAN。VLAN标记字段的前2个字节设置为0x8100（对应二进制10000001 00000000），称为**IEEE 802.1Q标记类型**；后2个字节被称为**标记控制信息**，前三位是用户优先级字段，接着一位是规范格式指示符（Canonical Format Indicator，CFI），剩下的12位组成了VLAN标识符（VLAN ID，VID）。这种MAC帧又被称作802.1Q帧。