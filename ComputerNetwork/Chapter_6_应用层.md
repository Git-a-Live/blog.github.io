...
对于任一的二进制文件，可以使用base64编码，即先把二进制代码按照24位一组划分成若干个单元，然后每个单元划分为4个6位组。6位二进制代码可以表示64种值，从0到63可以依次表示26个大写字母、26个小写字母、0-9共计10个数字以及表示62的`+`和63的`/`。最后再用`==`和`=`分别表示最后一个单元仅有8位和16位。base64编码忽略回车和换行。
