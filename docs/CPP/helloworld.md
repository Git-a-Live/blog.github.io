...
|类型|说明|
|:--|:--|
|char|通常 1 byte（8位），使用单引号`''`表示单个字符，特殊字符需要进行转义，可存储标准ASCII字符|
...
|string|以双引号`""`表示一个字符串，可以使用`+`以及`+=`进行字符串拼接，特殊字符同样需要进行转义，或是使用`R"(some_string)"`来表示原始的字符串|
